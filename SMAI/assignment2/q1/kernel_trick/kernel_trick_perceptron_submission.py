...
def get_input_data(filename):
    ...
    X = np.asarray(X); Y = np.asarray(Y, dtype=np.float)
...
def kernelize(X, Y):
    ...
    kernelX = np.hstack((X, np.ones((X.shape[0], 1))))
    n=1
    # kernelX[:, 2] = abs(kernelX[:, 0])**2 + abs(kernelX[:, 1])**2
    kernelX[:, 2] = kernelX[:, 0]**n + kernelX[:, 1]**n
    # for i,val in enumerate(kernelX):
        # kernelX[i,2] = exp(-1* (kernelX[i, 0]**n + kernelX[i, 1]**n)**0.5 / (2 * 1) )
